...
def hitterSearch(isFarm, teamChoice, hitterTargetChoice, hitterDataChoice, number, request):
    #urlをリスト形式で取得
    df_all = []
    years = range(19,9,-1)
    urls = []
    ...
    for year in years:
        if(year==19):
            urls.append('http://baseball-data.com/stats'+isFarm+'/hitter-'+teamChoice+'/tpa-'+hitterTargetChoice+'.html')
        else:
            urls.append('http://baseball-data.com/'+ "{0:02d}".format(year)+'/stats'+isFarm+'/hitter-'+teamChoice+'/tpa-'+hitterTargetChoice+'.html')
    ...
    for col in df_m.columns:
        if hitterDataChoice in col:
            data_col.append(col)
    df_m = df_m.sort_values(hitterDataChoice+'2019',ascending=False)
    df_m = df_m[data_col]
